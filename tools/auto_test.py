...
async def main():
    ...
    # ==================================================================
    # Phase 2: 提交用参数方程法的解答（替代方法 → 触发 RAG）
    # ==================================================================
    L("## Phase 2: 提交解题过程（参数方程法 — 触发替代方法 + RAG 检索）")
    L()

    work1 = (
        "椭圆 x²/4+y²/3=1，a²=4, b²=3, c²=1, 右焦点 F(1,0)。\n"
        "设直线 l: x=my+1，代入椭圆方程得 (my+1)²/4+y²/3=1。\n"
        "整理得 (3m²+4)y²+6my-9=0。\n"
        "设 A(x₁,y₁), B(x₂,y₂)，由韦达定理：\n"
        "y₁+y₂=-6m/(3m²+4), y₁y₂=-9/(3m²+4)。\n"
        "中点 M 的纵坐标 yₘ=(y₁+y₂)/2=-3m/(3m²+4)。\n"
        "xₘ=myₘ+1=m·(-3m/(3m²+4))+1=(4-3m²)... 这里我算不下去了"
    )
    L(f"> **学生提交**:")
    L()
    ...
    if session.mode.value == "socratic":
        ...
        responses = [
            "我算一下 xₘ：xₘ=m·yₘ+1=m·(-3m/(3m²+4))+1=(-3m²+3m²+4)/(3m²+4)=4/(3m²+4)",
            "所以中点 M 的坐标是 (4/(3m²+4), -3m/(3m²+4))，要消去参数 m 来求轨迹方程",
        ]
