...
Phase B/C 内容仓库 + CardStore + 方法目录 + 正式 solution_card_link   ⚙️ 骨架已完成
  ↓
Phase D/E 最小 concept 壳模型 + 全局 rag_audit_task + 双层 mastery    🔲 未开始
  ↓
...
- 🔲 实际知识卡内容填充 + `InMemoryCardStore` 载入
...
- 落地最小 concept 语义层：
  ...
  - `rag_audit_task`
- 把 `rag_audit_task` 设为全局权威任务源；现有会话态 `pending_audit_tasks` 只保留为镜像摘要。
- 先实现 problem/method 两层 mastery，thinking 层在 Phase G 前保持关闭。
- 让记忆、导出、审批链路都能显式区分题目目标、方法参考与 concept 链接状态。
...
- 🔲 Review `explain_concept` 接入 CardRetriever
- 🔲 Recommend 接入 CardRetriever
- 🔲 Memory 记录 `method_slot` / `solution_card_ids`
...
- M1.5：Phase D/E 完成，最小 concept 壳模型、`rag_audit_task`、双层 mastery 可用。 **🔲 未开始**
