...
from agent.infra.config import get_agent_params, settings
from agent.infra.logging import LLMStats, get_logger
from agent.infra.llm import (
...
from agent.infra.prompt import get_prompt_manager
...
class BaseAgent(ABC):
    ...
    def _count_message_chars(self, messages: list[dict[str, str]] | None) -> int:
        """粗略统计 messages 的文本体积。"""
        if not messages:
            return 0
        total = 0
    ...
    def _log_llm_observation(
        ...
        *,
        ...
        """记录与上下文签名关联的 LLM 调用日志。"""
        if not context_meta:
            return

        system_chars = len(system_prompt or "")
        user_chars = len(user_prompt or "")
        message_chars = self._count_message_chars(messages)
        input_chars = message_chars if messages else system_chars + user_chars
        ...
        record = {
            ...
            "context_signature": context_meta.get("context_signature"),
            "coverage_status": context_meta.get("coverage_status"),
            "degraded_mode": context_meta.get("degraded_mode"),
            "budget_utilization": context_meta.get("budget_utilization"),
            "context_chars": context_meta.get("final_char_count"),
            "context_token_estimate": context_meta.get("token_estimate"),
            "dropped_fields": context_meta.get("dropped_fields", []),
            "kept_fields": context_meta.get("kept_fields", []),
            "warnings": context_meta.get("warnings", []),
            "prompt_chars": {
                "system": system_chars,
                "user": user_chars,
                "messages": message_chars,
                "total": input_chars,
            },
        }
        if duration is not None:
            record["duration_ms"] = round(duration * 1000, 1)
        ...
        if error or context_meta.get("degraded_mode"):
